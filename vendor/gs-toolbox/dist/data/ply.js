...
export function readCompressedPLYBody(buffer, header, chunks) {
    ...
    // Compressed SH (optional)
    // SH data follows vertex packed data
    let shDegree = 0;
    let shN;
    // Check if there are SH properties beyond packed_*
    const shPropNames = properties
        .map(p => p.name)
        .filter(n => n.startsWith('f_rest_') || n.startsWith('f_dc_'));
    if (shPropNames.length > 0) {
        // Standard SH in compressed PLY — decode as uint8 → float
        const restProps = properties.filter(p => p.name.startsWith('f_rest_'));
        if (restProps.length > 0) {
            const numRest = restProps.length;
            // numRest = shCoeffs * 3 (total SH bytes)
            const shCoeffs = Math.floor(numRest / 3);
            if (shCoeffs > 0) {
                if (shCoeffs >= 15)
                    shDegree = 3;
                else if (shCoeffs >= 8)
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    for (let j = 0; j < numRest; j++) {
                        ...
                        const byte = dataView.getUint8(vertexBase + prop.offset);
                        shN[i * numRest + j] = decodeCompressedSH(byte);
                    }
                }
            }
        }
