 ...
 */
export function quatScaleToCovarPreciCPU(rotations, scales, count, options = {}) {
    const computeCovar = options.computeCovar ?? true;
    const computePreci = options.computePreci ?? true;
    const triu = options.triu ?? true;
    const covTriu = computeCovariance3D(rotations, scales, count);
    const covars = computeCovar ? (triu ? covTriu : expandTriuMatrices(covTriu, count)) : null;
    const precis = computePreci ? invertTriuMatrices(covTriu, count, triu) : null;
    return { covars, precis };
}
function expandTriuMatrices(triuMatrices, count) {
    const out = new Float32Array(count * 9);
    for (let i = 0; i < count; i++) {
        const ti = i * 6;
        const oi = i * 9;
        out[oi + 0] = triuMatrices[ti + 0];
        out[oi + 1] = triuMatrices[ti + 1];
        out[oi + 2] = triuMatrices[ti + 2];
        out[oi + 3] = triuMatrices[ti + 1];
        out[oi + 4] = triuMatrices[ti + 3];
        out[oi + 5] = triuMatrices[ti + 4];
        out[oi + 6] = triuMatrices[ti + 2];
        out[oi + 7] = triuMatrices[ti + 4];
        out[oi + 8] = triuMatrices[ti + 5];
    }
    return out;
}
function invertTriuMatrices(triuMatrices, count, triu) {
    const out = new Float32Array(count * (triu ? 6 : 9));
    for (let i = 0; i < count; i++) {
        const ti = i * 6;
        const a = triuMatrices[ti + 0];
        const b = triuMatrices[ti + 1];
        const c = triuMatrices[ti + 2];
        const d = triuMatrices[ti + 3];
        const e = triuMatrices[ti + 4];
        const f = triuMatrices[ti + 5];
        const co00 = d * f - e * e;
        const co01 = c * e - b * f;
        const co02 = b * e - c * d;
        const co11 = a * f - c * c;
        const co12 = b * c - a * e;
        const co22 = a * d - b * b;
        const det = a * co00 + b * co01 + c * co02;
        const invDet = Math.abs(det) > 1e-20 ? 1 / det : 0;
        if (triu) {
            const oi = i * 6;
            out[oi + 0] = co00 * invDet;
            out[oi + 1] = co01 * invDet;
            out[oi + 2] = co02 * invDet;
            out[oi + 3] = co11 * invDet;
            out[oi + 4] = co12 * invDet;
            out[oi + 5] = co22 * invDet;
        }
        else {
            const oi = i * 9;
            out[oi + 0] = co00 * invDet;
            out[oi + 1] = co01 * invDet;
            out[oi + 2] = co02 * invDet;
            out[oi + 3] = co01 * invDet;
            out[oi + 4] = co11 * invDet;
            out[oi + 5] = co12 * invDet;
            out[oi + 6] = co02 * invDet;
            out[oi + 7] = co12 * invDet;
            out[oi + 8] = co22 * invDet;
        }
    }
    return out;
}
